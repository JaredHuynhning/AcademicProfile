/**
 * 30 Learner Profile items (IDs 91-120).
 * Based on Grit-S (Duckworth), ASRS/APS-S (focus/procrastination),
 * SVS (vitality), TIMSS (subject attitudes), and MMCS (attribution).
 * All use 1-5 Likert scale: Strongly Disagree → Strongly Agree.
 *
 * Scoring metadata:
 *   domain    – 'grit' | 'focus' | 'energy' | 'subjectFit' | 'teacherPreference' | 'examBarriers'
 *   subscale  – specific construct within the domain
 *   reverse   – true if item is reverse-scored (score = 6 - raw)
 */
...
/**
 * Item lookup by ID for fast access.
 */
export const learnerItemMap = Object.fromEntries(learnerItems.map((item) => [item.id, item]));
