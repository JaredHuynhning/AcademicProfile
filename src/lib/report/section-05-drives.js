 ...
 */
import { isHigh, isLow, DIM_ORDER, DIM_SHORT } from './helpers.js';
import { analyzeXE } from './combinations.js';

export function generateDrives(results) {
	...
	return {
		...
		disengagement: {
			description: xe.disengage || 'Becomes passive and does the minimum required.',
...
function buildDemotivators(dims) {
	const demotivators = [];
...
function buildFlowTriggers(dims) {
	const triggers = [];
