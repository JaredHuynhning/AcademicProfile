...
interface FacetInsightResult {
	name: string;
	level: string;
	text: string;
}

export function generateDeepDive(results: Results) {
	return {
 ...
 */
function getFacetInsights(dimKey: string, facets: Record<string, { score: number; name?: string; level?: string }>): (FacetInsightResult | null)[] {
	const generators: Record<string, (f: Record<string, { score: number; name?: string; level?: string }>) => (FacetInsightResult | null)[]> = {
		H: (f) => [
		...
		E: (f) => [
			...
			facetInsight(f.dependence, 'Dependence',
				...
				'You are emotionally self-sufficient. You rarely lean on others for support. That is a strength in independence, but don\'t be afraid to ask for help.'
			),
			facetInsight(f.sentimentality, 'Sentimentality',
				...
				'You are emotionally detached and pragmatic. This helps you stay objective, but practice connecting with others\' feelings, as it builds stronger relationships.'
			)
		],
		X: (f) => [
			...
			facetInsight(f.sociability, 'Sociability',
				'You love being around people and draw energy from social interaction. Solo study may feel isolating, so build in regular social study time.',
...
function facetInsight(
	facet: { score: number; name?: string; level?: string } | undefined,
	name: string,
	highText: string,
	modText: string,
	lowText: string
): FacetInsightResult | null {
...
function getLearningCallout(
	dimKey: string,
	score: number,
	facets: Record<string, { score: number; name?: string; level?: string }>
): { title: string; text: string; icon: string } {
	const callouts: Record<string, () => { title: string; text: string; icon: string }> = {
		H: () => {
			if (score >= 3.5) return {
