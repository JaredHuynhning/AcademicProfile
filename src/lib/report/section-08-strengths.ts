...
export function generateStrengths(results: Results) {
	const allStrengths: (ClassifiedFacet & { score: string; rawScore: number; analysis: string; leverageTip: string })[] = [];
	const allWeaknesses: (ClassifiedFacet & { score: string; rawScore: number; challenge: string; actionTip: string })[] = [];

	const dimensions = DIM_ORDER.map((key) => {
		const dim = results.dimensions[key];
		const classified = classifyDimensionFacets(key, dim);

		const strengths = classified.strengths.map((f: ClassifiedFacet) => ({
			...f,
		...
		const weaknesses = classified.weaknesses.map((f: ClassifiedFacet) => ({
			...f,
			score: formatScore(f.score),
			rawScore: f.score,
			interpretLabel: interpretiveLabel(f.score),
		...
		const preferences = (classified.preferences || []).map((f: ClassifiedFacet) => ({
			...f,
			score: formatScore(f.score),
			rawScore: f.score,
			interpretLabel: interpretiveLabel(f.score),
...
function getStrengthAnalysis(facetKey: string, score: number): string {
...
function getLeverageTip(facetKey: string): string {
...
function getPreferenceDescription(facetKey: string, score: number): string {
...
function getWeaknessAnalysis(facetKey: string, score: number): string {
	const analyses: Record<string, string> = {
...
function getActionTip(facetKey: string): string {
	const tips: Record<string, string> = {
...
function buildWhatToDo(
	dimKey: string,
	strengths: { name: string; actionTip?: string }[],
	weaknesses: { name: string; actionTip: string }[],
	dim: { score: number; level: string }
): string {
...
function buildGrowthMindset(
	allStrengths: { key: string; name: string; rawScore: number }[],
	allWeaknesses: { key: string; name: string; rawScore: number }[]
) {
