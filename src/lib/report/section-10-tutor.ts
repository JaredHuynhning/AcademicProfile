...
function expandArchetype(archetype: Archetype, dims: DimensionsMap) {
...
function whatToLookFor(archetype: Archetype): string[] {
	const name = archetype.name?.toLowerCase() || '';
...
function getRedFlags(archetype: Archetype, dims: DimensionsMap): string[] {
	const flags: string[] = [];
...
function buildEvaluationTips(dims: DimensionsMap) {
	return [
		{
			tip: 'Trial session',
...
function buildGeneralAdvice(dims: DimensionsMap): string[] {
	const advice: string[] = [];
