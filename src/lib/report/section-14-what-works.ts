...
const TEACHER_PROFILES: Record<string, { label: string; icon: string; desc: string; tutorTip: string }> = {
	'warm-structured': {
...
const RESPONSE_TYPES: Record<string, { label: string; icon: string; desc: string; strategy: string }> = {
	encouragement: {
...
const BARRIER_DIAGNOSES: Record<string, { label: string; icon: string; desc: string; strategies: string[] }> = {
	anxiety: {
		label: 'Exam Anxiety',
		icon: '😰',
 ...
 */
export function generateWhatWorks(results: Results) {
	...
	const teacherInsights: { type: string; text: string }[] = [];
	...
	const responseInsights: { type: string; text: string }[] = [];
	...
	const examInsights: { type: string; text: string }[] = [];
