 ...
 */
import { isHigh, isLow, DIM_SHORT, rankDimensions, DimensionsMap } from './helpers';
...
function buildTeacherGuide(dims: DimensionsMap, ranked: { key: string; name: string; score: number; level: string }[]) {
	return {
...
function buildTeacherQuickProfile(dims: DimensionsMap): string[] {
	const traits: string[] = [];
...
function buildTeacherMotivation(dims: DimensionsMap) {
	const strategies: { approach: string; details: string }[] = [];
...
function buildFeedbackStyle(dims: DimensionsMap) {
...
function buildClassroomTips(dims: DimensionsMap): string[] {
	const tips: string[] = [];
...
function buildTeacherWarnings(dims: DimensionsMap) {
	const warnings: { signal: string; meaning: string; action: string }[] = [];
	if (isHigh(dims.E.score)) {
		warnings.push({
...
function buildParentGuide(dims: DimensionsMap, ranked: { key: string; name: string; score: number; level: string }[]) {
	return {
...
function buildParentProfile(dims: DimensionsMap, ranked: { key: string; name: string; score: number; level: string }[]): string {
...
function getParentExplanation(key: string, level: string): string {
...
function buildHomeEnvironment(dims: DimensionsMap) {
	const tips: { area: string; tip: string }[] = [];
	if (isHigh(dims.C.score)) {
...
function buildConversationStarters(dims: DimensionsMap) {
	const starters: { text: string; type: string }[] = [
...
function buildParentSupport(dims: DimensionsMap) {
	const strategies: { area: string; dos: string[]; donts: string[] }[] = [];
	strategies.push({
...
function buildParentWarnings(dims: DimensionsMap) {
	const warnings: { signal: string; action: string }[] = [];
	if (isHigh(dims.E.score)) {
		warnings.push({
